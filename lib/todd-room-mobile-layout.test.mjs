...
test("mobile activity status stays outside the 3D scene", async () => {
  ...
  assert.match(
    source,
    /data-mobile-activity-status="true"[\s\S]*?(md:hidden|lg:hidden)/,
    "the compact status strip should only render on smaller screens",
  );
  assert.match(
    source,
    /data-desktop-activity-overlay="true"[\s\S]*?hidden[\s\S]*?(md:block|lg:block)/,
